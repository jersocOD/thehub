...
def send_command(cmd: str):
    """Send one command to the Tello and log the reply."""
    print(f"[SEND] {cmd}")
    try:
...
# -------------------------------------------------------------------
# MJPEG generator for Person page (with detection + steering)
# -------------------------------------------------------------------
    ...
    while True:
        ...
        if frame is not None:
            ...
            if now - last_detect >= 1.0/DETECT_FPS:
                ...
                # steer toward largest if any
                if people:
                    x1,y1,x2,y2 = max(people, key=lambda b: (b[2]-b[0])*(b[3]-b[1]))
                    cx = (x1+x2)/2; bw = (x2-x1); offset = cx/W - 0.5
                    if abs(offset)>CENTER_TOL:
                        send_command('cw 15' if offset>0 else 'ccw 15')
...
<style>
:root {
  --bg:#0f172a;--panel:#1e293b;--accent:#4f46e5;--txt:#f8fafc;
...
header .counter{font-size:1.1rem;font-weight:500;}
main {
  display:grid;
  grid-template-columns:2fr 1fr;
  gap:1rem;
  padding:1rem;
  height:calc(100vh - 6rem); /* header+footer ~3rem each */
}
.video {
  position:relative;
  background:black;
  ...
  <div class="control-container">
    <div class="control">
      ...
      <div class="group"><h2>Special</h2><div class="grid2">
        <button onclick="location.href='/person'">👤 Person Detection</button>
        <div></div>
      </div></div>
    </div>
  </div>
